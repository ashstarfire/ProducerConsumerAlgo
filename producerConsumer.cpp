...
int main()
{
	...
	while(1)
	{
		...
		switch(n)
		{
			case 1:	if((mutex == 1)&&(empty != 0))  // Condition where no one is in critical section and buffer is not full.
						producer(buffer); // Calling Producer Function
					else
						cout << "\n\t\t\tBuffer is already full! You cannot produce more!" << endl;
					break;
			case 2:	if((mutex == 1)&&(full != 0)) // Condition where no one is in critical section and buffer is not empty
